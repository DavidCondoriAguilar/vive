...
const NavActions = ({ toggleMobileMenu, isMobileMenuOpen, waLink }) => {
    ...
    return (
        <div className="flex items-center gap-2 sm:gap-3 lg:gap-5 ml-auto flex-shrink-0">
            ...
            <a
                ...
                className="hidden lg:flex items-center justify-center w-10 h-10 rounded-xl text-gray-500 hover:text-green-500 hover:bg-gray-100 dark:hover:bg-white/10 transition-all duration-300"
                aria-label="WhatsApp"
            >
            ...
            {/* Mobile Toggle - Improved visibility and design */}
            <button
                onClick={toggleMobileMenu}
                className="lg:hidden w-11 h-11 flex items-center justify-center rounded-xl bg-gray-900 dark:bg-white text-white dark:text-gray-900 hover:shadow-lg active:scale-90 transition-all duration-300"
                aria-label="Menú"
                aria-expanded={isMobileMenuOpen}
                type="button"
            >
                <div className="w-5 flex flex-col items-center gap-1.5">
                    <span className={`h-0.5 bg-current transition-all duration-500 rounded-full ${isMobileMenuOpen ? 'w-5 translate-y-2 rotate-45' : 'w-5'}`} />
                    <span className={`h-0.5 bg-current transition-all duration-500 rounded-full ${isMobileMenuOpen ? 'opacity-0' : 'w-5'}`} />
                    <span className={`h-0.5 bg-current transition-all duration-500 rounded-full ${isMobileMenuOpen ? 'w-5 -translate-y-2 -rotate-45' : 'w-5'}`} />
                </div>
            </button>
