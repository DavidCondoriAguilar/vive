...
const EnhancedProductCatalog = () => {
    ...
    const categories = [
        ...
        { id: 'muebles', name: 'Variedad', count: products.filter(p => p.category === 'muebles').length }
    ];
                            ...
                            <div className="flex justify-center mt-12 space-x-2">
                                {Array.from({ length: Math.ceil(filteredProducts.length / itemsPerView) }).map((_, index) => (
                                    <button
                                        ...
                                        className={`w-2 h-2 rounded-full transition-colors duration-300 ${Math.floor(currentIndex / itemsPerView) === index
